...
static void Menu_Render(void) {
    ...
    if (current_page->layout == MENU_LAYOUT_CAROUSEL) {
        ...
        if (setting_show_scrollbar && current_page->item_count > 1) {
            int bar_w = 60;
            int bar_h = 2;
            int bar_x = (MLCD_WIDTH - bar_w) / 2;
            int bar_y = MLCD_HEIGHT - 6;
            
            // 轨道
            MLCD_DrawLine(bar_x, bar_y, bar_x + bar_w, bar_y, MLCD_COLOR_BLACK);
            ...
            MLCD_FillRect(thumb_x, bar_y - 1, thumb_w, bar_h + 2, MLCD_COLOR_BLACK);
        }
